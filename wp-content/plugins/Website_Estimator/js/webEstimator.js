...
//total
var jQuerytotal = jQuery("#W_E-total");


//sliders
var jQuerydaySlider = jQuery("#W_E-daysSlider");
var jQuerydatepicker = jQuery('#W_E-datepicker');
var jQuerydays = jQuery("#W_E-days");
var jQueryslideCalc = jQuery('#W_E-sliderCalc');
var jQuerypageSlider = jQuery("#W_E-pagesSlider");
var jQuerypages = jQuery("#W_E-pages");
var jQueryminusDays = jQuery("#W_E-minusDays");
var jQueryamount = jQuery("#W_E-amount");
var jQuerypages2 = jQuery("#W_E-pages2");
var jQueryplusDays = jQuery("#W_E-plusDays");
var jQueryminusPages = jQuery("#W_E-minusPages");
var jQueryplusPages = jQuery("#W_E-plusPages");

//CMS
var jQueryCMSAmount = jQuery("#W_E-CMS_AMOUNT");

//design
var jQuerydesignPrice = jQuery("#W_E-designPrice");
var jQuerydesignPrice2 = jQuery("#W_E-designPrice2");
var jQuerydeisgnYes = jQuery("#W_E-designYes");
var jQuerydesignHidden = jQuery('#W_E-designHidden');
var jQuerydesignOptions =  jQuery("#W_E-siteOptions");
var jQuerydesignNo = jQuery("#W_E-designNo");

//widgets
var jQuerywidgetsPrice = jQuery("#W_E-widgets");
...
/*--------------------------------------------------*/
	...
	jQuery('#W_E_TopMessage').hide();
	jQuery('#paypalBusiness').val(yourPayPalEmail);
	jQuery('.itemName').val(itemName);
	jQuery('.currencyCode').val(currencyCode);
	jQuery('#paymentPayPal').html(payPalButtonName);
...
jQuery('.payment').hide(); //--DO NOT EDIT

//WHILE form is being submitted loader
jQuery('.W_E_loader').hide();
	
	 //INITITIAL VALUES	
		jQueryCMSAmount.val(0);
		jQuerydesignOptions.val(0);
		jQuerywidgetsPrice.val(0);
        jQueryamount.val(pageAmount / 10);
		jQuerytotal.val(jQueryamount.val());
        jQuerypages.val(1);
        jQuerypages2.val(1);
        jQuerydays.val(9);
...
/*-------------------------------------------------*/
        ...
        jQuerydaySlider.slider({
                ...
                slide: function (event, ui) {
                        ...
                        jQuerydatepicker.datepicker('setDate', date); // And set into the datepicker	
                        jQuerydays.val(ui.value);
                        jQueryslideCalc.click();
                }
        });
        jQuerypageSlider.slider({
                animate: true,
                value: 0,
                min: 1,
                max: 16,
                step: 1,
                slide: function (event, ui) {
                        jQuerypages.val(ui.value);
                        jQueryslideCalc.click();
                }
        });
        jQueryminusDays.click(function () {
                if (jQuerydays.val() == 1) {
					alert('Please call for a speed estimate: ' + phoneNumber);
                        return false;
                } else {
                        var s = jQuerydaySlider,
                                val = s.slider("value"),
                        ...
                        jQuerydays.val(("value", val - step));
                        jQueryamount.val([(jQuerypages.val()) * pageAmount] / [("value", val - step)]);
                        jQueryslideCalc.click();
                }
        });
        jQueryplusDays.click(function () {
			if (jQuerydays.val() == 16) {
                        return false;
                } else {
                var s = jQuerydaySlider,
                        val = s.slider("value"),
        ...
        jQueryminusPages.click(function () {
                //Create logic to ignore click if #W_E-pages=0
                if (jQuerypages.val() == 1) {
                        return false;
                } else {
                        var s = jQuerypageSlider,
                                val = s.slider("value"),
                                step = s.slider("option", "step");
                        s.slider("value", val - step);
                        jQuerypages.val(("value", val - step));
                        jQueryamount.val([
                                [("value", val - step) * pageAmount] / [jQuerydaySlider.slider("value")]
                        ]);
                        jQueryslideCalc.click();
                }
        });
        jQueryplusPages.click(function () {
			 if (jQuerypages.val() == 16) {
                ...
                s.slider("value", val + step);
                jQuerypages.val(("value", val + step));
                jQueryamount.val([("value", val + step) * pageAmount] / [jQuerydaySlider.slider("value")]);
                jQueryslideCalc.click();
				}
        });
		
		jQueryslideCalc.click(function () {
				jQuerypages2 .val(parseInt(jQuerypages.val()));
                if (jQuerydays.val() >= maxDaysToReduceAmount) {
                        jQueryamount.val([(jQuerypages.val()) * pageAmount] / [maxDaysToReduceAmount]);
                        var total = parseInt(jQueryamount.val());
                        jQueryamount.val(total);
                } else {
                        jQueryamount.val([(jQuerypages.val()) * pageAmount] / [jQuerydays.val()]);
                        var total = parseInt(jQueryamount.val());
                        jQueryamount.val(total);
                }
        ...
        //Design
        jQuerydesignPrice.val("(" + currency + designPrice + ")");
        jQuerydesignPrice2.val(currency + 0);
        jQuerydeisgnYes.click(function () {
                jQuerydesignHidden.animate({
                        'height': '450px'
                }, 1000);
                jQuerydesignPrice2.val(currency + designPrice);
                var total = parseInt(jQuerydesignOptions.val());
                jQuerydesignOptions.val(total + designPrice);
                jQuerydeisgnYes.attr('disabled', 'disabled');
                jQuery("#W_E-designPrice2, #W_E-designPrice, #W_E-designAdded").addClass('W_E-added');
                jQuerydesignNo.removeAttr('disabled');
				jQuery("#designHidden").val('Yes');
        });
        jQuerydesignNo.click(function () {
                jQuerydesignHidden.animate({
                        'height': '0px'
                }, 1000);
                jQuerydesignPrice2.val(currency + 0);
                var total = parseInt(jQuerydesignOptions.val());
                jQuerydesignOptions.val(total - designPrice);
                jQuery("#W_E-designPrice2, #W_E-designPrice, #W_E-designAdded").removeClass('W_E-added');
                jQuerydeisgnYes.removeAttr('disabled');
                jQuerydesignNo.attr('disabled', 'disabled');
				jQuery("#designHidden").val('No');
        });
...
/*------------------------------------------------------*/
        ...
        jQuery("#W_E-cmsNone").click(function () {
				jQueryCMSAmount.val(cmsNone);
               
        });
        jQuery("#W_E-cms1").click(function () {
				jQueryCMSAmount.val(cms1Amount);
             
        });
		
        jQuery("#W_E-cms2").click(function () {
                jQueryCMSAmount.val(cms2Amount);
               
        });
        jQuery("#W_E-cms3").click(function () {
                jQueryCMSAmount.val(cms3Amount);
                
        });
        jQuery("#W_E-cms4").click(function () {
                jQueryCMSAmount.val(cms4Amount);
...
var jQueryW_E_extra1 = jQuery("#W_E-extra1");

        jQueryW_E_extra1.click(function () {
                if (jQueryW_E_extra1.is(':checked')) {
                        var total = parseInt(jQuerydesignOptions.val());
                        jQuerydesignOptions.val(Math.round([total + extra1Amount]));
                } else {
                        var total = parseInt(jQuerydesignOptions.val());
                        jQuerydesignOptions.val(Math.round([total - extra1Amount]));
                }
...
var jQueryW_E_extra2 = jQuery("#W_E-extra2");
        jQueryW_E_extra2.click(function () {
                if (jQueryW_E_extra2.is(':checked')) {
                        var total = parseInt(jQuerydesignOptions.val());
                        jQuerydesignOptions.val(Math.round([total + extra2Amount]));
                } else {
                        var total = parseInt(jQuerydesignOptions.val());
                        jQuerydesignOptions.val(Math.round([total - extra2Amount]));
                }
        });
var jQueryW_E_extra3 = jQuery("#W_E-extra3");
        jQueryW_E_extra3.click(function () {
                if (jQueryW_E_extra3.is(':checked')) {
                        var total = parseInt(jQuerydesignOptions.val());
                        jQuerydesignOptions.val(Math.round([total + extra3Amount]));
                } else {
                        var total = parseInt(jQuerydesignOptions.val());
                        jQuerydesignOptions.val(Math.round([total - extra3Amount]));
                }
        });
var jQueryW_E_extra4 = jQuery("#W_E-extra4");		
		jQueryW_E_extra4.click(function () {
                if (jQueryW_E_extra4.is(':checked')) {
                        var total = parseInt(jQuerydesignOptions.val());
                        jQuerydesignOptions.val(Math.round([total + extra4Amount]));
                } else {
                        var total = parseInt(jQuerydesignOptions.val());
                        jQuerydesignOptions.val(Math.round([total - extra4Amount]));
                }
        });
var jQueryW_E_extra5 = jQuery("#W_E-extra5");			
		jQueryW_E_extra5.click(function () {
                if (jQueryW_E_extra5.is(':checked')) {
                        var total = parseInt(jQuerydesignOptions.val());
                        jQuerydesignOptions.val(Math.round([total + extra5Amount]));
                } else {
                        var total = parseInt(jQuerydesignOptions.val());
                        jQuerydesignOptions.val(Math.round([total - extra5Amount]));
                }
        });
var jQueryW_E_extra6 = jQuery("#W_E-extra6");			
		jQueryW_E_extra6.click(function () {
                if (jQueryW_E_extra6.is(':checked')) {
                        var total = parseInt(jQuerydesignOptions.val());
                        jQuerydesignOptions.val(Math.round([total + extra6Amount]));
                } else {
                        var total = parseInt(jQuerydesignOptions.val());
                        jQuerydesignOptions.val(Math.round([total - extra6Amount]));
                }
        });
var jQueryW_E_extra7 = jQuery("#W_E-extra7");			
		jQueryW_E_extra7.click(function () {
                if (jQueryW_E_extra7.is(':checked')) {
                        var total = parseInt(jQuerydesignOptions.val());
                        jQuerydesignOptions.val(Math.round([total + extra7Amount]));
                } else {
                        var total = parseInt(jQuerydesignOptions.val());
                        jQuerydesignOptions.val(Math.round([total - extra7Amount]));
                }
        });
var jQueryW_E_extra8 = jQuery("#W_E-extra8");			
		jQueryW_E_extra8.click(function () {
                if (jQueryW_E_extra8.is(':checked')) {
                        var total = parseInt(jQuerydesignOptions.val());
                        jQuerydesignOptions.val(Math.round([total + extra8Amount]));
                } else {
                        var total = parseInt(jQuerydesignOptions.val());
                        jQuerydesignOptions.val(Math.round([total - extra8Amount]));
                }
        });
var jQueryW_E_extra9 = jQuery("#W_E-extra9");			
		jQueryW_E_extra9.click(function () {
                if (jQueryW_E_extra9.is(':checked')) {
                        var total = parseInt(jQuerydesignOptions.val());
                        jQuerydesignOptions.val(Math.round([total + extra9Amount]));
                } else {
                        var total = parseInt(jQuerydesignOptions.val());
                        jQuerydesignOptions.val(Math.round([total - extra9Amount]));
                }
        });
var jQueryW_E_extra10 = jQuery("#W_E-extra10");	
		jQueryW_E_extra10.click(function () {
                if (jQueryW_E_extra10.is(':checked')) {
                        var total = parseInt(jQuerydesignOptions.val());
                        jQuerydesignOptions.val(Math.round([total + extra10Amount]));
                } else {
                        var total = parseInt(jQuerydesignOptions.val());
                        jQuerydesignOptions.val(Math.round([total - extra10Amount]));
                }
...
/*-------------------------------------------------*/
        //IMAGE GALLERY
var jQueryW_E_imgGalleryPrice = jQuery("#W_E-imgGalleryPrice");
var jQueryW_E_imgGalleryPrice2 =  jQuery("#W_E-imgGalleryPrice2");
var jQueryW_E_imgGalleryYes =  jQuery("#W_E-imgGalleryYes");
var jQueryW_E_imgGalleryNo =  jQuery("#W_E-imgGalleryNo");

        jQueryW_E_imgGalleryPrice.val("(" + currency + imgGalleryPrice + ")");
        jQueryW_E_imgGalleryPrice2.val(currency + 0);
        jQueryW_E_imgGalleryYes.click(function () {
                jQueryW_E_imgGalleryPrice2.val(currency + imgGalleryPrice);
                var total = parseInt(jQuerywidgetsPrice.val());
                jQuerywidgetsPrice.val(total + imgGalleryPrice);
                jQueryW_E_imgGalleryYes.attr('disabled', 'disabled');
                jQuery("#W_E-imgGalleryPrice2, #W_E-imgGalleryPrice, #W_E-imgGalleryAdded").addClass('W_E-added');
                jQueryW_E_imgGalleryNo.removeAttr('disabled');
				jQuery("#imgGalleryHidden").val('Yes');
        });
        jQueryW_E_imgGalleryNo.click(function () {
                jQueryW_E_imgGalleryPrice2.val(currency + 0);
                var total = parseInt(jQuerywidgetsPrice.val());
                jQuerywidgetsPrice.val(total - imgGalleryPrice);
                jQuery("#W_E-imgGalleryPrice2, #W_E-imgGalleryPrice, #W_E-imgGalleryAdded").removeClass('W_E-added');
                jQueryW_E_imgGalleryYes.removeAttr('disabled');
                jQueryW_E_imgGalleryNo.attr('disabled', 'disabled');
				jQuery("#imgGalleryHidden").val('No');
        });
		
		
        //SLIDESHOW
var jQueryW_E_slideShowPrice =  jQuery("#W_E-slideShowPrice");
var jQueryW_E_slideShowPrice2 =  jQuery("#W_E-slideShowPrice2");
var jQueryW_E_slideShowYes =  jQuery("#W_E-slideShowYes");
var jQueryW_E_slideShowNo =  jQuery("#W_E-slideShowNo");

        jQueryW_E_slideShowPrice.val("(" + currency + slideShowPrice + ")");
        jQueryW_E_slideShowPrice2.val(currency + 0);
        jQueryW_E_slideShowYes.click(function () {
                jQueryW_E_slideShowPrice2.val(currency + slideShowPrice);
                var total = parseInt(jQuerywidgetsPrice.val());
                jQuerywidgetsPrice.val(total + slideShowPrice);
                jQuery("#W_E-slideShowPrice2, #W_E-slideShowPrice, #W_E-slideShowAdded").addClass('W_E-added');
                jQueryW_E_slideShowYes.attr('disabled', 'disabled');
                jQueryW_E_slideShowNo.removeAttr('disabled');
				jQuery("#slideShowHidden").val('Yes');
        });
        jQueryW_E_slideShowNo.click(function () {
                jQueryW_E_slideShowPrice2.val(currency + 0);
                var total = parseInt(jQuerywidgetsPrice.val());
                jQuerywidgetsPrice.val(total - slideShowPrice);
                jQuery("#W_E-slideShowPrice2, #W_E-slideShowPrice, #W_E-slideShowAdded").removeClass('W_E-added');
                jQueryW_E_slideShowYes.removeAttr('disabled');
                jQueryW_E_slideShowNo.attr('disabled', 'disabled');
				jQuery("#slideShowHidden").val('No');
        });
		
		
        //LIGHTBOX
var jQueryW_E_lightBoxPrice =  jQuery("#W_E-lightBoxPrice");
var jQueryW_E_lightBoxPrice2 = jQuery("#W_E-lightBoxPrice2");
var jQueryW_E_lightBoxYes =  jQuery("#W_E-lightBoxYes");
var jQueryW_E_lightBoxNo =  jQuery("#W_E-lightBoxNo");		

		
        jQueryW_E_lightBoxPrice.val("(" + currency + lightBoxPrice + ")");
        jQueryW_E_lightBoxPrice2.val(currency + 0);
        jQueryW_E_lightBoxYes.click(function () {
                jQueryW_E_lightBoxPrice2.val(currency + lightBoxPrice);
                var total = parseInt(jQuerywidgetsPrice.val());
                jQuerywidgetsPrice.val(total + lightBoxPrice);
                jQuery("#W_E-lightBoxPrice2, #W_E-lightBoxPrice, #W_E-lightBoxAdded").addClass('W_E-added');
                jQueryW_E_lightBoxYes.attr('disabled', 'disabled');
                jQueryW_E_lightBoxNo.removeAttr('disabled');
				jQuery("#lightboxHidden").val('Yes');
        });
        jQueryW_E_lightBoxNo.click(function () {
                jQueryW_E_lightBoxPrice2.val(currency + 0);
                var total = parseInt(jQuerywidgetsPrice.val());
                jQuerywidgetsPrice.val(total - lightBoxPrice);
                jQuery("#W_E-lightBoxPrice2, #W_E-lightBoxPrice, #W_E-lightBoxAdded").removeClass('W_E-added');
                jQueryW_E_lightBoxYes.removeAttr('disabled');
                jQueryW_E_lightBoxNo.attr('disabled', 'disabled');
				jQuery("#lightboxHidden").val('No');
        });
		
		
        //SHARETHIS
		
var jQueryW_E_shareThisPrice =  jQuery("#W_E-shareThisPrice");
var jQueryW_E_shareThisPrice2 = jQuery("#W_E-shareThisPrice2");
var jQueryW_E_shareThisYes =  jQuery("#W_E-shareThisYes");
var jQueryW_E_shareThisNo =  jQuery("#W_E-shareThisNo");	
		
        jQueryW_E_shareThisPrice.val("(" + currency + shareThisPrice + ")");
        jQueryW_E_shareThisPrice2.val(currency + 0);
        jQueryW_E_shareThisYes.click(function () {
                jQueryW_E_shareThisPrice2.val(currency + shareThisPrice);
                var total = parseInt(jQuerywidgetsPrice.val());
                jQuerywidgetsPrice.val(total + shareThisPrice);
                jQuery("#W_E-shareThisPrice2, #W_E-shareThisPrice, #W_E-shareThisAdded").addClass('W_E-added');
                jQueryW_E_shareThisYes.attr('disabled', 'disabled');
                jQueryW_E_shareThisNo.removeAttr('disabled');
				jQuery("#shareThisHidden").val('Yes');
        });
        jQueryW_E_shareThisNo.click(function () {
                jQueryW_E_shareThisPrice2.val(currency + 0);
                var total = parseInt(jQuerywidgetsPrice.val());
                jQuerywidgetsPrice.val(total - shareThisPrice);
                jQuery("#W_E-shareThisPrice2, #W_E-shareThisPrice, #W_E-shareThisAdded").removeClass('W_E-added');
                jQueryW_E_shareThisYes.removeAttr('disabled');
                jQueryW_E_shareThisNo.attr('disabled', 'disabled');
				jQuery("#shareThisHidden").val('No');
        });
		
		 //Carousel
		 
var jQueryW_E_carouselPrice =  jQuery("#W_E-carouselPrice");
var jQueryW_E_carouselPrice2 = jQuery("#W_E-carouselPrice2");
var jQueryW_E_carouselYes =  jQuery("#W_E-carouselYes");
var jQueryW_E_carouselNo =  jQuery("#W_E-carouselNo");
		 
        jQueryW_E_carouselPrice.val("(" + currency + carouselPrice + ")");
        jQueryW_E_carouselPrice2.val(currency + 0);
        jQueryW_E_carouselYes.click(function () {
                jQueryW_E_carouselPrice2.val(currency + carouselPrice);
                var total = parseInt(jQuerywidgetsPrice.val());
                jQuerywidgetsPrice.val(total + carouselPrice);
                jQuery("#W_E-carouselPrice2, #W_E-carouselPrice, #W_E-carouselAdded").addClass('W_E-added');
                jQueryW_E_carouselYes.attr('disabled', 'disabled');
                jQueryW_E_carouselNo.removeAttr('disabled');
				jQuery("#carouselHidden").val('Yes');
        });
        jQueryW_E_carouselNo.click(function () {
                jQueryW_E_carouselPrice2.val(currency + 0);
                var total = parseInt(jQuerywidgetsPrice.val());
                jQuerywidgetsPrice.val(total - carouselPrice);
                jQuery("#W_E-carouselPrice2, #W_E-carouselPrice, #W_E-carouselAdded").removeClass('W_E-added');
                jQueryW_E_carouselYes.removeAttr('disabled');
                jQueryW_E_carouselNo.attr('disabled', 'disabled');
				jQuery("#carouselHidden").val('No');
        });
		
		 //Accordion
		 
var jQueryW_E_accordionPrice =  jQuery("#W_E-accordionPrice");
var jQueryW_E_accordionPrice2 = jQuery("#W_E-accordionPrice2");
var jQueryW_E_accordionYes =  jQuery("#W_E-accordionYes");
var jQueryW_E_accordionNo =  jQuery("#W_E-accordionNo");
		 
        jQueryW_E_accordionPrice.val("(" + currency + accordionPrice + ")");
        jQueryW_E_accordionPrice2.val(currency + 0);
        jQueryW_E_accordionYes.click(function () {
                jQueryW_E_accordionPrice2.val(currency + accordionPrice);
                var total = parseInt(jQuerywidgetsPrice.val());
                jQuerywidgetsPrice.val(total + accordionPrice);
                jQuery("#W_E-accordionPrice2, #W_E-accordionPrice, #W_E-accordionAdded").addClass('W_E-added');
                jQueryW_E_accordionYes.attr('disabled', 'disabled');
                jQueryW_E_accordionNo.removeAttr('disabled');
				jQuery("#accordionHidden").val('Yes');
        });
        jQuery("#W_E-accordionNo").click(function () {
                jQueryW_E_accordionPrice2.val(currency + 0);
                var total = parseInt(jQuerywidgetsPrice.val());
                jQuerywidgetsPrice.val(total - accordionPrice);
                jQuery("#W_E-accordionPrice2, #W_E-accordionPrice, #W_E-accordionAdded").removeClass('W_E-added');
                jQueryW_E_accordionYes.removeAttr('disabled');
                jQueryW_E_accordionNo.attr('disabled', 'disabled');
				jQuery("#accordionHidden").val('No');
        });
		
		 //Custom Forms
		 
var jQueryW_E_customFormsPrice =  jQuery("#W_E-customFormsPrice");
var jQueryW_E_customFormsPrice2 = jQuery("#W_E-customFormsPrice2");
var jQueryW_E_customFormsYes =  jQuery("#W_E-customFormsYes");
var jQueryW_E_customFormsNo =  jQuery("#W_E-customFormsNo");
		 
        jQueryW_E_customFormsPrice.val("(" + currency + customFormsPrice + ")");
        jQueryW_E_customFormsPrice2.val(currency + 0);
        jQueryW_E_customFormsYes.click(function () {
                jQueryW_E_customFormsPrice2.val(currency + customFormsPrice);
                var total = parseInt(jQuerywidgetsPrice.val());
                jQuerywidgetsPrice.val(total + customFormsPrice);
                jQuery("#W_E-customFormsPrice2, #W_E-customFormsPrice, #W_E-customFormsAdded").addClass('W_E-added');
                jQueryW_E_customFormsYes.attr('disabled', 'disabled');
                jQueryW_E_customFormsNo.removeAttr('disabled');
				jQuery("#customFormsHidden").val('Yes');
        });
        jQueryW_E_customFormsNo.click(function () {
                jQueryW_E_customFormsPrice2.val(currency + 0);
                var total = parseInt(jQuerywidgetsPrice.val());
                jQuerywidgetsPrice.val(total - customFormsPrice);
                jQuery("#W_E-customFormsPrice2, #W_E-customFormsPrice, #W_E-customFormsAdded").removeClass('W_E-added');
                jQueryW_E_customFormsYes.removeAttr('disabled');
                jQueryW_E_customFormsNo.attr('disabled', 'disabled');
				jQuery("#customFormsHidden").val('No');
        });
		
		 //Flyout Menu
var jQueryW_E_flyOutMenuPrice =  jQuery("#W_E-flyOutMenuPrice");
var jQueryW_E_flyOutMenuPrice2 = jQuery("#W_E-flyOutMenuPrice2");
var jQueryW_E_flyOutMenuYes =  jQuery("#W_E-flyOutMenuYes");
var jQueryW_E_flyOutMenuNo =  jQuery("#W_E-flyOutMenuNo");
		 
        jQueryW_E_flyOutMenuPrice.val("(" + currency + flyOutMenuPrice + ")");
        jQueryW_E_flyOutMenuPrice2.val(currency + 0);
        jQueryW_E_flyOutMenuYes.click(function () {
                jQueryW_E_flyOutMenuPrice2.val(currency + flyOutMenuPrice);
                var total = parseInt(jQuerywidgetsPrice.val());
                jQuerywidgetsPrice.val(total + flyOutMenuPrice);
                jQuery("#W_E-flyOutMenuPrice2, #W_E-flyOutMenuPrice, #W_E-flyOutMenuAdded").addClass('W_E-added');
                jQueryW_E_flyOutMenuYes.attr('disabled', 'disabled');
                jQueryW_E_flyOutMenuNo.removeAttr('disabled');
				jQuery("#flyoutMenuHidden").val('Yes');
        });
        jQueryW_E_flyOutMenuNo.click(function () {
                jQueryW_E_flyOutMenuPrice2.val(currency + 0);
                var total = parseInt(jQuerywidgetsPrice.val());
                jQuerywidgetsPrice.val(total - flyOutMenuPrice);
                jQuery("#W_E-flyOutMenuPrice2, #W_E-flyOutMenuPrice, #W_E-flyOutMenuAdded").removeClass('W_E-added');
                jQueryW_E_flyOutMenuYes.removeAttr('disabled');
                jQueryW_E_flyOutMenuNo.attr('disabled', 'disabled');
				jQuery("#flyoutMenuHidden").val('No');
        });
		

		jQuery(document).bind('click', function(){
			
		var amountForTotal = parseInt(jQueryamount.val());
		var cmsForTotal = parseInt(jQuery('#W_E-CMS_AMOUNT').val());
		var siteOptionsForTotal = parseInt(jQuery('#W_E-siteOptions').val());
		var widgetsForTotal = parseInt(jQuery('#W_E-widgets').val());
		
		jQuerytotal.val(amountForTotal + cmsForTotal + siteOptionsForTotal + widgetsForTotal);
		jQuery('.payPalAmount').val(amountForTotal + cmsForTotal + siteOptionsForTotal + widgetsForTotal);
        ...
        //---------ACCORDION--------------
        //-------------------------------------
        jQuery("#W_E-accordion").accordion({
                autoHeight: false,
                collapsible: true
        });
      
		jQuery('#W_E-extra1txt').html(extra1txt + '<br />');
		jQuery('#W_E-extra2txt').html(extra2txt + '<br />');
		jQuery('#W_E-extra3txt').html(extra3txt + '<br />');
		jQuery('#W_E-extra4txt').html(extra4txt + '<br />');
		jQuery('#W_E-extra5txt').html(extra5txt + '<br />');
		jQuery('#W_E-extra6txt').html(extra6txt + '<br />');
		jQuery('#W_E-extra7txt').html(extra7txt + '<br />');
		jQuery('#W_E-extra8txt').html(extra8txt + '<br />');
		jQuery('#W_E-extra9txt').html(extra9txt + '<br />');
		jQuery('#W_E-extra10txt').html(extra10txt + '<br />');
}
